    ...
    <main className="flex-1 flex flex-col">
      ...
      <div className="flex-1 px-4 sm:px-6 py-8 max-w-3xl mx-auto w-full">
        ...
        <p className="text-white/60 text-sm mb-8">
          Stored locally in this browser. Sign-in &amp; cross-device sync coming later.
        </p>
        ...
        {history.length === 0 ? (
          ...
          <>
            ...
            <h2 className="font-serif text-xl font-black text-gold-bright mb-3">
              History
            </h2>
            ...
            <h2 className="font-serif text-xl font-black text-gold-bright mt-8 mb-3">
              By category
            </h2>
            <div className="bg-board-deep rounded p-4 space-y-2">
              {stats.categories.map((c) => (
                <div key={c.tag} className="flex items-center gap-3">
                  <div className="w-32 text-sm">{c.tag}</div>
...
function computeStats(history: HistoryEntry[]) {
  ...
  const avg = Math.round(history.reduce((s, h) => s + h.finalScore, 0) / games);

  // Simple streak: consecutive days ending today (Eastern).
